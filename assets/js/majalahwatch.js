...
function loadApp() {
  ...
  function daftarIsi(judul, nomor) {
    let divbar = $("<div class=' '>");
    let p = $("<p>");
    let h = $("<h1>");
    h.text(judul);
    p.text(nomor);
    h.addClass("ml-5 mt-1 font-medium capitalize");
    p.addClass("font-extrabold text-3xl lg:mr-1 ");
    divbar.append(p);
    divbar.append(h);
    divbar.addClass(
      "flex justify-start item-center py-3 cursor-pointer hover:bg-white/70 active:bg-white/50 px-3 "
    );
  ...
  function timeoutas() {
    timeoutid = setTimeout(function () {
      $("#navbar").addClass("-translate-y-[70px]");
      tanda2 = 1;
    }, 5000);
  }
  ...
  $("#hideNav").click(function () {
    $("#navbar").toggleClass("-translate-y-[70px]");
  });

  $("#navbar").mouseover(function () {
    $("#navbar").removeClass("-translate-y-[70px]");
  });
...
//   let inst = range.value;
//   let inst2 = parseInt(inst);
//   zoomScale = inst2;
//   if(inst2>1){
//     $(".magazine-viewport").zoom("zoomIn");
//   }
